-- ═══════════════════════════════════════════════════════════════════════════
-- Madger · Phase 2 : paiement en 3 fois (Klarna) sur les PACKS à partir de
-- 120 €, au choix du coach (les frais Klarna, plus élevés que la carte, sont
-- déduits de son versement comme les frais Stripe).
-- À exécuter dans Supabase → SQL Editor → Run (après 0059).
-- ═══════════════════════════════════════════════════════════════════════════

alter table public.coaches
  add column if not exists installments_enabled boolean not null default false;
grant update (installments_enabled) on public.coaches to authenticated;

-- ── Vue publique : la fiche coach annonce le 3x sur les packs éligibles ─────
...
as $$
  ...
  select id, slug, first_name, last_name, specialty, bio, avatar_url,
         ...
         from_price_cents, verified, cancel_hours, installments_enabled
    from candidates
