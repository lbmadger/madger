...
export default function SidebarProfile() {
  const { email, name, avatarUrl, slug, pro, clientSpace } = useSession();
  const { t } = useI18n();
    ...
    <div className="relative" ref={ref}>
      {open && (
        <div className="absolute bottom-full left-0 z-30 mb-2 w-full overflow-hidden rounded-xl border border-border bg-bg-elevated shadow-xl">
          ...
          {clientSpace && (
            <Link href="/espace" onClick={() => setOpen(false)} className={itemCls}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
          ...
          {/* Un coach qui bloque doit trouver quelqu'un en deux clics. */}
          <a
