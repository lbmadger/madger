        ...
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="border-b border-border bg-bg-card text-left text-xs uppercase tracking-wide text-text-dim">
              ...
              <th className="whitespace-nowrap px-4 py-3">Coach</th>
            </tr>
          </thead>
          <tbody>
            {(clients ?? []).map((c) => {
              ...
              return (
                <tr key={c.id as string} className="border-b border-border/60">
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-text-muted">{c.email || "-"}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-text-muted">{c.phone || "-"}</td>
